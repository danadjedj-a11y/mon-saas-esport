...
-- 2. Ajouter phase_id aux matchs existants
...
CREATE POLICY "Bracket slots are editable by tournament owner" ON bracket_slots
  FOR ALL USING (
    EXISTS (
      ...
      WHERE p.id = bracket_slots.phase_id 
      AND t.owner_id = auth.uid()
    )
  );
...
CREATE POLICY "Custom data is viewable by participant or owner" ON participant_custom_data
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM participants p
      JOIN tournaments t ON t.id = p.tournament_id
      WHERE p.id = participant_custom_data.participant_id
      AND (p.user_id = auth.uid() OR t.owner_id = auth.uid())
    )
  );

CREATE POLICY "Custom data is editable by participant or owner" ON participant_custom_data
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM participants p
      JOIN tournaments t ON t.id = p.tournament_id
...
-- Fonction helper pour mettre à jour updated_at
