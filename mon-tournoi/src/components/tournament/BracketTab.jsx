    ...
    <div className="bg-gray-900/95 p-8 rounded-xl border-2 border-cyan-400 shadow-lg shadow-violet-500/30">
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-white">
          <thead>
            <tr className="bg-violet-500/30 text-left border-b-2 border-cyan-400">
              <th className="p-3 rounded-tl font-handwriting text-cyan-400">Rang</th>
              <th className="p-3 font-handwriting text-cyan-400">Équipe</th>
              <th className="p-3 text-center font-handwriting text-cyan-400">Pts</th>
              <th className="p-3 text-center font-handwriting text-cyan-400">J</th>
              <th className="p-3 text-center font-handwriting text-cyan-400">V</th>
              <th className="p-3 text-center font-handwriting text-cyan-400">N</th>
              <th className="p-3 text-center rounded-tr font-handwriting text-cyan-400">D</th>
            </tr>
          </thead>
          <tbody>
            {standings.map((team, index) => (
              <tr key={team.id} className="border-b border-cyan-400/30">
                <td className={`p-3 font-display text-lg ${index === 0 ? 'font-bold text-cyan-400' : 'text-white'}`}>
                  #{index + 1}
                </td>
                <td className="p-3">
                ...
                <td className="p-3 text-center font-bold text-xl text-cyan-400 font-handwriting">
                  {team.points}
                </td>
...
function SwissStandings({ swissScores, participants }) {
  ...
  return (
    <div className="bg-gray-900/95 p-8 rounded-xl border-2 border-cyan-400 shadow-lg shadow-violet-500/30 mb-10">
      <h2 className="mt-0 text-cyan-400 font-handwriting text-3xl mb-6">
        🇨🇭 Classement Suisse
      </h2>
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-white">
          <thead>
            <tr className="bg-violet-500/30 text-left border-b-2 border-cyan-400">
              <th className="p-3 rounded-tl font-handwriting text-cyan-400">Rang</th>
              <th className="p-3 font-handwriting text-cyan-400">Équipe</th>
              <th className="p-3 text-center font-handwriting text-cyan-400">Victoires</th>
              <th className="p-3 text-center font-handwriting text-cyan-400">Défaites</th>
              <th className="p-3 text-center font-handwriting text-cyan-400">Nuls</th>
              <th className="p-3 text-center rounded-tr font-handwriting text-cyan-400">Buchholz</th>
            </tr>
          </thead>
          <tbody>
            {sortedScores.map((score, index) => {
              ...
              return (
                <tr key={score.id} className="border-b border-cyan-400/30">
                  <td className={`p-3 font-display text-lg ${index === 0 ? 'font-bold text-cyan-400' : 'text-white'}`}>
                    #{index + 1}
                  </td>
                  <td className="p-3">
                    <div className="flex items-center gap-2.5">
                      <img 
                        ...
                        loading="lazy"
                      />
                      <span className={`font-display ${index === 0 ? 'font-bold' : ''} text-white`}>
                        {team?.teams?.name || 'Inconnu'}
                      </span>
                    </div>
                  </td>
                  <td className="p-3 text-center font-bold text-lg text-cyan-400 font-handwriting">
                    {score.wins}
                  </td>
                  <td className="p-3 text-center font-display text-white">{score.losses}</td>
                  <td className="p-3 text-center font-display text-white">{score.draws}</td>
                  <td className="p-3 text-center font-bold text-cyan-400 font-handwriting">
    ...
    <div className="flex gap-10 pb-5 min-w-fit">
      {rounds.map(round => (
        <div key={round} className="flex flex-col justify-around gap-5">
          <h4 className="text-center text-white mb-4 font-display">
...
export default function BracketTab({ tournoi, matches, swissScores, participants, getStandings }) {
  const emptyState = (
    <div className="text-center p-12 border-2 border-dashed border-cyan-400 rounded-lg bg-gray-900/50 text-white font-display">
  ...
  if (tournoi.format === 'swiss') {
    return (
      <div>
        ...
        {matches.length > 0 && (
          <div className="bg-gray-900/95 p-8 rounded-xl border-2 border-cyan-400 shadow-lg shadow-violet-500/30 overflow-x-auto">
            <h2 className="mt-0 text-cyan-400 font-handwriting text-3xl mb-6">
              🇨🇭 Rounds
            </h2>
            <BracketRounds 
      ...
      <div className="bg-gray-900/95 p-8 rounded-xl border-2 border-cyan-400 shadow-lg shadow-violet-500/30 overflow-x-auto">
        ...
        <div className="flex gap-10 pb-5 min-w-fit">
          ...
          <div className="flex-1">
            <h3 className="text-center text-cyan-400 mb-5 font-handwriting text-2xl">
              🏆 Winners Bracket
            </h3>
          ...
          {/* Losers Bracket */}
          <div className="flex-1">
            <h3 className="text-center text-violet-400 mb-5 font-handwriting text-2xl">
              💀 Losers Bracket
            </h3>
            ...
            {grandFinals.length > 0 && (
              <div className="mt-10 pt-5 border-t-4 border-cyan-400">
                <h3 className="text-center text-cyan-400 mb-5 font-handwriting text-2xl">
                  🏅 Grand Finals
                </h3>
            ...
            {/* Reset Match */}
            {resetMatch.length > 0 && (
              <div className="mt-5 pt-5 border-t-4 border-cyan-400">
                <h4 className="text-center text-orange-400 mb-2.5 font-handwriting text-xl">
                  🔄 Reset Match
                </h4>
                <div className="flex justify-center">
                  {resetMatch.map(m => (
